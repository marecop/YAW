...
export default function BookingDetailPage() {
  ...
  const fetchBookingDetails = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('預訂不存在')
      }
      const data = await response.json()
      setBooking(data)
    } catch (err: any) {
      setError(err.message || '載入預訂資料失敗')
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">找不到預訂</h2>
          <p className="text-gray-600 mb-6">{error || '該預訂不存在或已被刪除'}</p>
          <Link
            href="/zh-hk/bookings"
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">預訂詳情</h1>
              <p className="text-gray-600">預訂編號：{booking.bookingNumber}</p>
            </div>
          ...
          <div className="relative">
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex flex-col items-center px-4">
                <div className="text-sm text-gray-600 mb-2">飛行時間</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {booking.passengerEmail && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="text-sm text-gray-600">電子郵件</div>
            ...
            {booking.passportNumber && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="text-sm text-gray-600">護照號碼</div>
        ...
        {/* 价格信息 */}
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">價格明細</h2>
          ...
          <div className="space-y-3">
            ...
            <div className="border-t border-gray-200 pt-3 mt-3">
              <div className="flex justify-between items-center">
                <span className="text-lg font-bold text-gray-900">總價</span>
        ...
        <div className="flex flex-col gap-4">
          {booking.status === 'CONFIRMED' && !booking.checkedIn && (() => {
            ...
            if (hoursUntilDeparture > 48) {
              ...
              return (
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg text-center">
                  ...
                  <p className="text-xs text-blue-600 mt-1">
                    還需 {daysUntilOpen > 0 ? `${daysUntilOpen}天 ` : ''}{remainingHours}小時 開放
                  </p>
                </div>
              )
              ...
              return (
                <Link
                  href={`/zh-hk/check-in?bookingNumber=${booking.bookingNumber}&email=${booking.passengerEmail}`}
          ...
          {booking.checkedIn && (
            <Link
              href={`/zh-hk/boarding-pass/${booking.id}`}
              target="_blank"
        ...
        <div className="mt-6 text-center text-sm text-gray-500">
          預訂時間：{formatDate(booking.createdAt)} {formatTime(booking.createdAt)}
        </div>
      </div>
    </div>
  )
