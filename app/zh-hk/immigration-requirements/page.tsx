...
export default function ImmigrationRequirementsPage() {
  ...
  const currentLabels = {
    ...
    loading: '查詢中...',
    sameCountry: '無需行動',
    sameCountryDesc: '您正在前往您的祖國或擁有居留權的地區。',
    health: '檢疫 / 健康申報',
    customs: '海關申報',
    entryForm: '入境卡'
  }
  ...
  const getRuleNotes = (r: ImmigrationRule) => {
    return r.notesZhHk || r.notesZhCn || r.notesEn
  }
  
  const getHealthInfo = (r: ImmigrationRule) => r.healthZhHk || r.healthZhCn || r.healthEn || '-'
  const getCustomsInfo = (r: ImmigrationRule) => r.customsZhHk || r.customsZhCn || r.customsEn || '-'
  const getEntryFormInfo = (r: ImmigrationRule) => r.entryFormZhHk || r.entryFormZhCn || r.entryFormEn || '-'
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                         <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                           <div className="flex items-center mb-2 text-blue-800">
                             ...
                             <span className="font-semibold">{currentLabels.health}</span>
                           </div>
                           <p className="text-sm text-blue-900">{getHealthInfo(rule)}</p>
                         </div>
                         <div className="bg-purple-50 p-4 rounded-lg border border-purple-100">
                           <div className="flex items-center mb-2 text-purple-800">
                             <Briefcase className="h-5 w-5 mr-2" />
                             <span className="font-semibold">{currentLabels.customs}</span>
                           </div>
                           <p className="text-sm text-purple-900">{getCustomsInfo(rule)}</p>
                         </div>
                         <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100">
                           <div className="flex items-center mb-2 text-indigo-800">
                             <FileText className="h-5 w-5 mr-2" />
                             <span className="font-semibold">{currentLabels.entryForm}</span>
                           </div>
                           <p className="text-sm text-indigo-900">{getEntryFormInfo(rule)}</p>
                         </div>
                      </div>
