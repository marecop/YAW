...
function FlightStatusContent() {
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case 'SCHEDULED': return { label: 'Plan', color: 'bg-gray-100 text-gray-800', icon: Clock }
      case 'PREPARING': return { label: 'Vorbereitung', color: 'bg-blue-50 text-blue-600', icon: Clock }
      case 'DELAYED': return { label: 'Verzögert', color: 'bg-yellow-100 text-yellow-800', icon: AlertTriangle }
      case 'CANCELLED': return { label: 'Abgesagt', color: 'bg-red-100 text-red-800', icon: XCircle }
      case 'BOARDING': return { label: 'Boardung', color: 'bg-green-100 text-green-800', icon: Plane }
      case 'GATE_CLOSED': return { label: 'Gate geschlossen', color: 'bg-orange-100 text-orange-800', icon: Clock }
      case 'IN_AIR': return { label: 'Flug', color: 'bg-blue-100 text-blue-800', icon: Plane }
      case 'ARRIVED': return { label: 'Gelandet', color: 'bg-green-100 text-green-800', icon: CheckCircle }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Flugstatus-Dashboard</h1>
          <p className="mt-2 text-gray-600">Realtime Display of Today's Flight Status Information</p>
          <div className="mt-2 text-sm text-gray-500">
            Letzte Aktualisierung: {format(lastUpdated, 'HH:mm:ss')}
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg shadow mb-6 flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
                <label htmlFor="search" className="block text-sm font-medium text-gray-700 mb-1">Flug suchen</label>
                <input
                    type="text"
                    id="search"
                    placeholder="Flugnummer / Stadt / Flughafencode"
            ...
            <div className="sm:w-64">
                <label htmlFor="status" className="block text-sm font-medium text-gray-700 mb-1">Status filtern</label>
                <select
                    id="status"
                    className="block w-full rounded-md border-gray-300 shadow-sm focus:border-ya-yellow-500 focus:ring-ya-yellow-500 sm:text-sm p-2 border"
                    value={statusFilter}
                ...
                >
                    ...
                    <option value="SCHEDULED">Plan (Scheduled)</option>
                    <option value="PREPARING">Vorbereitung (Preparing)</option>
                    <option value="BOARDING">Boardung (Boarding)</option>
                    <option value="GATE_CLOSED">Gate geschlossen (Gate Closed)</option>
                    <option value="IN_AIR">Flug (In Air)</option>
                    <option value="ARRIVED">Gelandet (Arrived)</option>
                    <option value="DELAYED">Verzögert (Delayed)</option>
                    <option value="CANCELLED">Abgesagt (Cancelled)</option>
                </select>
            </div>
        </div>
                  ...
                  {flights.length > 0 ? (
                      ...
                      return (
                        <tr key={flight.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <Link href={`/de/flight-status/${flight.id}`} className="text-ya-yellow-600 hover:text-ya-yellow-900">
                              Details
                            </Link>
                          </td>
                        </tr>
                    ...
                    <tr>
                        <td colSpan={7} className="px-6 py-10 text-center text-gray-500">
                            Keine Fluginformationen gefunden
                        </td>
                    </tr>
                  )}
                </tbody>
