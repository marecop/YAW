...
export default function FlightStatusDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case 'SCHEDULED': return 'Plan'
      case 'PREPARING': return 'Vorbereitung'
      case 'DELAYED': return 'Verzögert'
      case 'CANCELLED': return 'Abgesagt'
      case 'BOARDING': return 'Boardung'
      case 'GATE_CLOSED': return 'Gate geschlossen'
      case 'IN_AIR': return 'Flug'
      case 'ARRIVED': return 'Gelandet'
  ...
  if (!flight) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-xl text-gray-500">Keine Fluginformationen gefunden</p>
      </div>
    )
  }
            ...
            <div className="absolute bottom-8 left-4 right-4 md:left-auto md:right-8 md:w-[500px] z-[1000] shadow-2xl rounded-xl overflow-hidden bg-white/95 backdrop-blur-sm border border-gray-100">
                ...
                <div className="p-6">
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm border-t border-gray-100 pt-4">
                        <div>
                            <div className="text-gray-500 mb-1">Geplanter Start</div>
                            <div className="font-semibold text-lg">{format(new Date(flight.scheduledDeparture), 'HH:mm')}</div>
                        </div>
                        <div className="text-right">
                            <div className="text-gray-500 mb-1">Geplanter Ankunft</div>
                            <div className="font-semibold text-lg">
                                ...
                                {new Date(flight.scheduledArrival).getDate() !== new Date(flight.scheduledDeparture).getDate() && (
                                    <span className="text-xs text-ya-yellow-600 font-bold ml-1 align-topt(">+1 Tag</span>
                                )}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            {(flight.status === 'IN_AIR' || flight.status === 'ARRIVED') ? (
                                <>
                                    <div className=")text-gray-500 mb-1">Tatsächlicher Start</div>
                        ...
                        <div className="text-right">
                            ...
                            {flight.status === 'ARRIVED' ? (
                                <>
                                    <div className="text-gray-500 mb-1">Tatsächliche Ankunft</div>
                    ...
                    <div className="mt-6 pt-4 border-t border-gray-100 flex justify-between items-center text-xs text-gray-500">
                        <div>
                            <span className="font-medium text-gray-700">Modell:</span> {flight.aircraftType}
                        </div>
                        <div>
                            <span className="font-medium text-gray-700">Registrierungsnummer:</span> {flight.aircraftRegistration}
                        </div>
                        <div>
                            <span className="font-medium text-gray-700">Wetter:</span> {flight.weatherOrigin} → {flight.weatherDestination}
                        </div>
                    </div>
                </div>
            </div>
