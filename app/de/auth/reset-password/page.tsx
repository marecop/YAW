...
function ResetPasswordContent() {
  ...
  const verifyToken = async (tokenToVerify: string) => {
    try {
      ...
      if (!response.ok) {
        setError(data.message || 'Datenbankfehler, bitte versuchen Sie es später erneut')
      }
    } catch (err) {
      setTokenValid(false)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 8) {
      setError('Passwortlänge muss mindestens 8 Zeichen sein')
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Passwort zurücksetzen fehlgeschlagen')
      }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Verifizierung des Passwort-Zurücksetzen-Links...</p>
        </div>
      </div>
    )
  }
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-gray-600">
              {error || 'Der Link zum Zurücksetzen des Passworts ist ungültig oder abgelaufen. Bitte wenden Sie sich erneut an den Support.'}
            </p>
          </div>
          ...
          <div className="mt-6 space-y-3">
            ...
            >
              Link zum Zurücksetzen des Passworts erneut anfordern
            </Link>
            <Link
              href="/de/auth/login"
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="text-xs text-gray-500">
            <p className="mb-1">Passwortanforderungen:</p>
            <ul className="list-disc list-inside space-y-1">
              <li>Mindestens 8 Zeichen</li>
              <li>Groß- und Kleinschreibung enthalten</li>
