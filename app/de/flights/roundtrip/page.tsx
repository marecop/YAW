...
function RoundtripSearchContent() {
  ...
  const fetchFlights = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || 'Suchen von Rundreiseflügen fehlgeschlagen')
      }
    } catch (err) {
  ...
  const handleContinue = () => {
    if (!selectedOutbound || !selectedInbound) {
      alert('Bitte wählen Sie den Hin- und Rückflug')
      return
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-gray">
        <div className="text-center">
          ...
          <p className="text-xl text-navy font-semibold">Laden...</p>
          <p className="text-text-secondary mt-2">Suche nach den besten Rundreiseflügen...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-gray">
        <div className="card-modern p-8 max-w-md text-center">
          ...
          <h2 className="text-2xl font-bold text-navy mb-2">Fehler</h2>
