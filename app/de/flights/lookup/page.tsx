...
function FlightLookupContent() {
  ...
  const currentLabels = {
    title: 'Flugnummer suchen',
    subtitle: 'Flugnummer suchen um Flugzeiten und Status zu finden.',
    placeholder: 'Flugnummer eingeben (z.B. YA101)',
    dateLabel: 'Datum',
    search: 'Suchen',
    ...
    schedule: 'Geplanter Zeit',
    depTime: 'Startzeit',
    arrTime: 'Ankunftzeit',
    actions: 'Aktionen',
    checkStatus: 'Flugstatus ansehen',
    bookTicket: 'Flug buchen',
    notFound: 'Keine Fluginformationen gefunden.',
    enterNumber: 'Bitte Flugnummer eingeben.',
  }
  ...
  const lookup = async (num: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError('Suchen fehlgeschlagen, bitte versuchen Sie es später erneut.')
      }
    } catch (e) {
              ...
              <div className="p-6">
                {!operatesOnSelectedDate && (
                  <div className="rounded-lg bg-yellow-50 p-4 mb-6 text-sm text-yellow-800 border border-yellow-200">
                    Dieser Flug ist möglicherweise nicht am ausgewählten Tag in Betrieb (basierend auf dem tatsächlichen Zeitplan/Lufthansa-Ausschreibung).
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 border-t border-gray-100">
                  <Link 
                    href={`/de/flight-status?search=${encodeURIComponent(result.flightNumber)}&date=${encodeURIComponent(selectedDate)}`}
                  ...
                  <Link 
                    href={`/de/flights/search?from=${encodeURIComponent(result.from)}&to=${encodeURIComponent(result.to)}&departureDate=${encodeURIComponent(selectedDate)}`}
