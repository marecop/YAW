...
export default function FlightSearchPage() {
  ...
  useEffect(() => {
    const fetchFlights = async () => {
      ...
      try {
        ...
        if (!directRes.ok) {
          throw new Error(directData.error || '搜索失败')
        }
  ...
  const handleSelectCabin = (flightId: string, cabinType: string) => {
    router.push(`/de/flights/${flightId}/book?from=${from}&to=${to}&departureDate=${selectedDate || departureDate}&passengers=${passengers}&cabinClass=${cabinType}`)
  }
  ...
  const handleDateChange = (newDate: string) => {
    ...
    router.push(`/de/flights/search?${newParams.toString()}`, { scroll: false })
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-yellow-50 py-12 px-4">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-text-secondary">Suche nach Flügen...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-yellow-50">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-sm text-text-secondary">Abfahrt</div>
                  <div className="font-medium text-navy">
                    {format(new Date(selectedDate || departureDate), 'MM月dd日 (E)', { locale: require('date-fns/locale/zh-HK') })}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-sm text-text-secondary">Passagiere</div>
                  <div className="font-medium text-navy">{passengers} Personen</div>
                </div>
              </div>
            </div>
          </div>
                  ...
                  .map(([dateStr, { date, minPrice }]) => (
                    ...
                    >
                      <div className={`text-xs ${dateStr === (selectedDate || departureDate) ? 'text-white' : 'text-text-secondary'}`}>
                        {format(date, 'd日E', { locale: require('date-fns/locale/zh-HK') })}
                      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          >
            <option value="departure">Sortieren nach: Am relevantesten</option>
            <option value="price">Am günstigsten</option>
            <option value="duration">Am kürzesten</option>
          </select>
        </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex-1 px-4 text-center">
                          ...
                          <div className="flex items-center">
                            ...
                            <div className="text-xs text-text-secondary px-2">Direkt</div>
                            <div className="flex-1 h-px bg-border"></div>
                          </div>
                        </div>
                        ...
                        >
                          <div className="space-y-2">
                            <div className={`text-xs font-semibold uppercase ${cabin.available ? 'text-navy' : 'text-gray-400'}`}>
                              {cabin.type === 'ECONOMY' && 'Economy'}
                              {cabin.type === 'PREMIUM_ECONOMY' && 'Premium Economy'}
                              {cabin.type === 'BUSINESS' && 'Business'}
                              {cabin.type === 'FIRST_CLASS' && 'First Class'}
                            </div>
        ...
        {filteredConnections.length > 0 && (
          <div className="space-y-4">
            <h2 className="text-xl font-bold text-navy mb-4">Umstiege</h2>
        ...
        {filteredFlights.length === 0 && filteredConnections.length === 0 && !isLoading && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-bold text-navy mb-2">Keine Flüge gefunden</h3>
            <p className="text-text-secondary">Bitte versuchen Sie es mit anderen Suchkriterien</p>
          </div>
        )}
      </div>
    </div>
