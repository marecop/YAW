...
export default function FlightStatusDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case 'SCHEDULED': return '定刻'
      case 'PREPARING': return '準備中'
      case 'DELAYED': return '遅延'
      case 'CANCELLED': return '欠航'
      case 'BOARDING': return '搭乗中'
      case 'GATE_CLOSED': return '搭乗締切'
  ...
  const getDiffDisplay = (scheduled: Date, actual: Date) => {
    ...
    if (diffMins > 0) {
        return <span className="text-red-600 text-xs font-bold ml-2">(+{diffMins}分)</span>
    } else {
        return <span className="text-green-600 text-xs font-bold ml-2">({diffMins}分)</span>
    }
  }
  ...
  if (!flight) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-xl text-gray-500">フライト情報が見つかりません</p>
      </div>
    )
  }
            ...
            <div className="absolute bottom-8 left-4 right-4 md:left-auto md:right-8 md:w-[500px] z-[1000] shadow-2xl rounded-xl overflow-hidden bg-white/95 backdrop-blur-sm border border-gray-100">
                ...
                <div className="p-6">
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm border-t border-gray-100 pt-4">
                        ...
                        <div>
                            ...
                            {(flight.status === 'IN_AIR' || flight.status === 'ARRIVED') ? (
                                <>
                                    <div className="text-gray-500 mb-1">実際出発</div>
                                ...
                                <>
                                    <div className="text-gray-500 mb-1">予想出発</div>
                        ...
                        <div className="text-right">
                            ...
                            {flight.status === 'ARRIVED' ? (
                                <>
                                    <div className="text-gray-500 mb-1">実際到着</div>
                                ...
                                <>
                                    <div className="text-gray-500 mb-1">予想到着</div>
                    ...
                    <div className="mt-6 pt-4 border-t border-gray-100 flex justify-between items-center text-xs text-gray-500">
                        <div>
                            <span className="font-medium text-gray-700">機材:</span> {flight.aircraftType}
                        </div>
                        <div>
                            <span className="font-medium text-gray-700">登録番号:</span> {flight.aircraftRegistration}
                        </div>
                        <div>
                            <span className="font-medium text-gray-700">天気:</span> {flight.weatherOrigin} → {flight.weatherDestination}
                        </div>
                    </div>
                </div>
            </div>
