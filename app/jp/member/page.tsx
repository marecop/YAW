...
export default function MemberPage() {
    ...
    if (user && !pointsUpdated) {
      ...
      fetch('/api/cron/process-points')
        ...
        .then(data => {
          ...
          if (data.processed > 0) {
            // ポイントの更新がある場合、ページを更新して最新のポイントを表示
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      router.push('/jp/auth/login?redirect=/member')
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-xl text-gray-700 font-semibold">読み込み中...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10">
          ...
          <p className="text-gray-500">アカウントと予約の管理</p>
        </div>
          ...
          <div className="lg:col-span-1">
            ...
            <div className={`rounded-2xl overflow-hidden bg-gradient-to-br ${getMembershipColor(user.membershipLevel)} p-8 text-white mb-8 shadow-xl relative`}>
              ...
              <div className="mb-8 relative z-10">
                <p className="text-xs font-medium opacity-80 mb-1 uppercase">会員名</p>
                <p className="text-xl font-semibold tracking-wide">{user.name}</p>
              </div>

              <div className="flex justify-between items-end relative z-10">
                <div>
                  <p className="text-xs font-medium opacity-80 mb-1 uppercase">ポイント</p>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              <h3 className="font-bold text-gray-900 mb-4 px-2">クイックアクション</h3>
              <div className="space-y-1">
                ...
                >
                  ...
                  <span className="text-gray-700 font-medium flex-1">予約確認</span>
                ...
                >
                  ...
                  <span className="text-gray-700 font-medium flex-1">アカウント設定</span>
                  <ChevronRight className="w-4 h-4 text-gray-400 group-hover:translate-x-1 transition-transform" />
                </Link>
                <Link
                  href="/jp/member/rewards"
                  className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors group"
                >
                  ...
                  <span className="text-gray-700 font-medium flex-1">ポイントモール</span>
                  <ChevronRight className="w-4 h-4 text-gray-400 group-hover:translate-x-1 transition-transform" />
                </Link>
                <button
                ...
                >
                  ...
                  <span className="font-medium text-left flex-1">ログアウト</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12">
                ...
                <div>
                  <p className="text-sm text-gray-500 mb-1">会員ランク</p>
                ...
                <div>
                  <p className="text-sm text-gray-500 mb-1">利用可能ポイント</p>
            ...
            {/* Membership Benefits */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">会員特典</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-6 bg-gray-50 rounded-2xl hover:bg-yellow-50 transition-colors duration-300 group">
                  ...
                  <h3 className="font-bold text-gray-900 mb-2 text-lg">ポイントボーナス</h3>
                  <p className="text-sm text-gray-600 leading-relaxed">
                    {user.membershipLevel === 'PLATINUM' && 'フライトごとにポイント2.0倍'}
                    {user.membershipLevel === 'GOLD' && 'フライトごとにポイント1.5倍'}
                    {user.membershipLevel === 'SILVER' && 'フライトごとにポイント1.25倍'}
                  </p>
                </div>
                <div className="p-6 bg-gray-50 rounded-2xl hover:bg-yellow-50 transition-colors duration-300 group">
                  <div className="w-12 h-12 bg-white rounded-xl shadow-sm flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                  ...
                  <h3 className="font-bold text-gray-900 mb-2 text-lg">優先サービス</h3>
                  <p className="text-sm text-gray-600 leading-relaxed">
                    {user.membershipLevel === 'PLATINUM' && '優先チェックイン、優先搭乗、ラウンジ利用'}
                    {user.membershipLevel === 'GOLD' && '優先搭乗、ラウンジ利用'}
                    {user.membershipLevel === 'SILVER' && '優先チェックイン'}
                  </p>
                </div>
              </div>
              <div className="mt-8 text-center">
                <Link
                  href="/jp/member/privileges"
                  className="inline-flex items-center text-ya-yellow-600 font-semibold hover:text-ya-yellow-700 transition-colors group"
                >
                  会員特典の詳細を見る <ArrowRight className="w-4 h-4 ml-1 group-hover:translate-x-1 transition-transform" />
                </Link>
              </div>
            </div>
          </div>
