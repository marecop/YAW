...
export default function RewardsPage() {
  ...
  const handleRedeem = async (reward: Reward) => {
    ...
    if (!confirm(`${reward.points.toLocaleString()} ポイントを使用して「${reward.title}」を交換しますか？`)) {
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        alert(`交換しました！${reward.title} がアカウントに追加されました。`)
        // ポイントを更新するためにユーザー情報を更新
        if (refreshUser) {
            await refreshUser()
        } else {
            // refreshUserが利用できない場合、一時的にページを手動で更新するか、何もしない
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">ポイントモール</h1>
            <p className="text-gray-500">ポイントを使って素敵な特典と交換しましょう</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-gray-500">利用可能ポイント</p>
              <p className="text-3xl font-bold text-gray-900">{user?.points?.toLocaleString() || 0}</p>
            </div>
          </div>
        </div>

        <h2 className="text-xl font-bold text-gray-900 mb-6">人気の特典</h2>
              ...
              <div key={reward.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow group flex flex-col h-full">
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-100">
                    <span className="text-ya-yellow-600 font-bold">{reward.points.toLocaleString()} ポイント</span>
                    <button 
