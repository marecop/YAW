...
function LoginForm() {
  ...
  // 如果已登录，重定向（但要等待 AuthContext 初始化完成）
  useEffect(() => {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          アカウントをお持ちでないですか？{' '}
          <Link href="/jp/auth/register" className="font-medium text-ya-yellow-600 hover:text-ya-yellow-500">
            新規登録
          </Link>
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm">
                <a href="/jp/auth/forgot-password" className="font-medium text-ya-yellow-600 hover:text-ya-yellow-500">
                  パスワードをお忘れですか？
                </a>
              </div>
            </div>
          ...
          <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm font-medium text-blue-900 mb-2">デモアカウント</p>
            <p className="text-xs text-blue-700">メール: john@example.com</p>
            <p className="text-xs text-blue-700">パスワード: password123</p>
          </div>
        </div>
      </div>
    </div>
