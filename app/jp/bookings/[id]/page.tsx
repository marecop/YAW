...
export default function BookingDetailPage() {
  ...
  const fetchBookingDetails = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('予約が存在しません')
      }
      const data = await response.json()
      setBooking(data)
    } catch (err: any) {
      setError(err.message || '予約情報の読み込みに失敗しました')
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">予約が見つかりません</h2>
          <p className="text-gray-600 mb-6">{error || 'この予約は存在しないか削除されました'}</p>
          <Link
            href="/jp/bookings"
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">予約詳細</h1>
              <p className="text-gray-600">予約番号：{booking.bookingNumber}</p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              <div className="text-sm text-gray-600 mb-1">便名</div>
            ...
            <div>
              ...
              {booking.seatNumber && (
                <div className="text-sm text-gray-600">座席：{booking.seatNumber}</div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {booking.passportNumber && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="text-sm text-gray-600">パスポート番号</div>
        ...
        {/* 価格情報 */}
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">価格詳細</h2>
              ...
              return (
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg text-center">
                  ...
                  <p className="text-sm text-blue-800 font-medium">
                    オンラインチェックインは出発48時間前から開始されます
                  </p>
                  <p className="text-xs text-blue-600 mt-1">
                    開始まであと {daysUntilOpen > 0 ? `${daysUntilOpen}日 ` : ''}{remainingHours}時間
                  </p>
                </div>
              )
        ...
        <div className="mt-6 text-center text-sm text-gray-500">
          予約日時：{formatDate(booking.createdAt)} {formatTime(booking.createdAt)}
        </div>
      </div>
    </div>
  )
