...
interface Country {
  ...
  nameZhCn: string;
  nameZhHk: string;
  nameJp?: string; // Added optional JP name if available in API, otherwise fallback
}
...
interface ImmigrationRule {
  ...
  notesZhCn?: string;
  notesZhHk?: string;
  notesJp?: string; // Assuming API might support this or we fallback
}
...
export default function ImmigrationRequirementsPage() {
  ...
  const currentLabels = {
    title: '出入国要件の確認',
    subtitle: '国籍と目的地に基づいてビザの要件を確認します。',
    passportLabel: 'パスポート / 国籍',
    destLabel: '目的地',
    ...
    loading: '確認中...',
    sameCountry: 'アクション不要',
    sameCountryDesc: '母国または居住権のある地域への渡航です。'
  }

  // Helper to get localized name
  const getCountryName = (c: Country) => {
    // Fallback order: JP -> EN -> CN
    return c.nameJp || c.nameEn
  }

  // Helper to get localized notes
  const getRuleNotes = (r: ImmigrationRule) => {
    return r.notesJp || r.notesEn || r.notesZhHk
  }
