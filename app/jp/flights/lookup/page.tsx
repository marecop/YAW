...
function FlightLookupContent() {
  ...
  const currentLabels = {
    title: 'フライト番号検索',
    subtitle: 'フライト番号でスケジュールと運航状況を確認できます。',
    placeholder: 'フライト番号を入力 (例: YA101)',
    dateLabel: '日付',
    search: '検索',
    flightInfo: 'フライト情報',
    origin: '出発地',
    ...
    actions: '操作',
    checkStatus: '運航状況を確認',
    bookTicket: 'このフライトを予約',
    notFound: '該当するフライト番号のフライトが見つかりません。',
    enterNumber: 'フライト番号を入力してください。',
  }
              ...
              <div className="p-6">
                {!operatesOnSelectedDate && (
                  <div className="rounded-lg bg-yellow-50 p-4 mb-6 text-sm text-yellow-800 border border-yellow-200">
                    選択された日付にこのフライトは運航していない可能性があります（実際のスケジュール/航空会社の発表を確認してください）。
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 border-t border-gray-100">
                  <Link 
                    href={`/jp/flight-status?search=${encodeURIComponent(result.flightNumber)}&date=${encodeURIComponent(selectedDate)}`}
                  ...
                  <Link 
                    href={`/jp/flights/search?from=${encodeURIComponent(result.from)}&to=${encodeURIComponent(result.to)}&departureDate=${encodeURIComponent(selectedDate)}`}
