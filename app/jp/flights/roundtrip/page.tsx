  ...
  const fetchFlights = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || '往復フライトの検索に失敗しました')
      }
    } catch (err) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
          ...
          <div className="lg:col-span-4 flex flex-col items-end justify-center gap-4 lg:border-l lg:pl-6">
            <div className="text-right">
              <p className="text-sm text-text-secondary mb-1">合計金額</p>
            ...
            <div className="flex flex-col lg:flex-row items-center gap-3 w-full">
              ...
              >
                <option value="ECONOMY">エコノミークラス</option>
                <option value="BUSINESS">ビジネスクラス</option>
                <option value="FIRST_CLASS">ファーストクラス</option>
              </select>
              <button
                onClick={(e) => {
                  e.stopPropagation()
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-gray">
        <div className="text-center">
          ...
          <p className="text-xl text-navy font-semibold">読み込み中...</p>
          <p className="text-text-secondary mt-2">最適な往復フライトを検索中...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-gray">
        <div className="card-modern p-8 max-w-md text-center">
          ...
          <h2 className="text-2xl font-bold text-navy mb-2">エラーが発生しました</h2>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-gray">
        <div className="card-modern p-12 max-w-md text-center">
          ...
          <h2 className="text-2xl font-bold text-navy mb-2">条件に一致するフライトが見つかりません</h2>
          <p className="text-text-secondary mb-6">
            {outboundFlights.length === 0 ? '往路のフライトがありません' : '復路のフライトがありません'}
          </p>
          <button
            onClick={() => router.push('/jp/')}
            className="px-6 py-3 bg-primary text-navy font-semibold rounded-lg hover:bg-primary-dark transition-colors"
  ...
  return (
    <div className="min-h-screen bg-background-gray py-8">
      <div className="container-custom">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-navy mb-4">往復フライトを選択</h1>
          ...
          {selectedOutbound && selectedInbound && (
            <div className="card-modern p-6 sticky bottom-6 bg-white shadow-2xl">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                  <p className="text-sm text-text-secondary mb-1">合計金額 ({passengers} 名)</p>
