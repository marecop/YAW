...
export default function BaggagePage() {
  ...
  const excessBaggageFees = [
    { weight: '23-32公斤（超重）', economy: 700, business: 500, first: 0 },
    { weight: '額外行李（每件23公斤以下）', economy: 1200, business: 1000, first: 800 },
    { weight: '超大尺寸行李（159-203厘米）', economy: 1500, business: 1200, first: 1000 },
  ]
  ...
  const restrictedItems = [
    {
      ...
      items: [
        ...
        '攻擊性武器（槍支、刀具等）',
        '麻醉品和違禁藥物',
        '沒有埃森蘭國家標準證書及大於（不包括）20000mAh的充電寶，鋰電池',
        '所有液體在安檢前都不允許攜帶，包括水、飲料、湯、液體食物等。噴霧及凝膠僅可攜帶總量不超過100毫升。',
        '未經申報的無線通訊工具（包括無線網絡設備，對講機，無人機，無線電台等。）',
        '含有酒精的個人護理用品（酒精含量50%以上）',
      ]
    },
    {
      ...
      items: [
        ...
        '攻擊性武器（槍支、大型管制刀具等）',
        '麻醉品和違禁藥物',
        '大於（不包括）20000mAh的充電寶，鋰電池',
        '大量液體、凝膠和噴霧（每個容器超過100毫升，總量超過1升）'
      ]
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        '嬰兒食品（適量）',
        '處方藥物（攜帶處方或醫生證明）',
        '經過申報的無線通訊工具（無線網絡設備，對講機，無人機，無線電台等。）',
        '酒精含量少於50%個人護理用品。'
      ]
    },
    {
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {specialBaggageFees.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {item.fee === null ? '根據重量計費' : `HK$${item.fee} / 件`}
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {excessBaggageFees.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {item.economy === 0 ? '免費' : `HK$${item.economy}`}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {item.business === 0 ? '免費' : `HK$${item.business}`}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {item.first === 0 ? '免費' : `HK$${item.first}`}
                      </td>
                    </tr>
                  ))}
                </tbody>
