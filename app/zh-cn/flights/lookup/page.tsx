...
function FlightLookupContent() {
  ...
  const currentLabels = {
    title: '航班號查詢',
    subtitle: '通過航班號查詢航班時刻表及狀態。',
    placeholder: '輸入航班號 (如 YA101)',
    dateLabel: '日期',
    search: '查詢',
    flightInfo: '航班資訊',
    origin: '出發地',
            ...
            <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden animate-fade-in">
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 border-t border-gray-100">
                  <Link 
                    href={`/zh-cn/flight-status?search=${encodeURIComponent(result.flightNumber)}&date=${encodeURIComponent(selectedDate)}`}
                  ...
                  <Link 
                    href={`/zh-cn/flights/search?from=${encodeURIComponent(result.from)}&to=${encodeURIComponent(result.to)}&departureDate=${encodeURIComponent(selectedDate)}`}
