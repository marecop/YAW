...
function RoundtripSearchContent() {
  ...
  const fetchFlights = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || 'Error al buscar vuelos de ida y vuelta')
      }
    } catch (err) {
      setError('Error de red, por favor intente nuevamente más tarde')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleContinue = () => {
    if (!selectedOutbound || !selectedInbound) {
      alert('Por favor seleccione vuelos de ida y vuelta')
      return
    }
    ...
    router.push(`/es/booking/passengers?${params.toString()}`)
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
          ...
          <div className="lg:col-span-8">
            ...
            <div className="mt-4 text-sm text-text-secondary">
              Asientos restantes: {flight.availableSeats}
            </div>
          </div>
          ...
          <div className="lg:col-span-4 flex flex-col items-end justify-center gap-4 lg:border-l lg:pl-6">
            <div className="text-right">
              <p className="text-sm text-text-secondary mb-1">Precio total</p>
              <p className="text-4xl font-bold text-primary">
                ¥{cabinPrice.toLocaleString()}
              </p>
              <p className="text-xs text-text-secondary mt-1">
                ¥{(cabinPrice/passengers).toLocaleString()} / persona
              </p>
            </div>
            
            {/* Cabin Selection and Select Button */}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-gray">
        <div className="text-center">
          ...
          <p className="text-xl text-navy font-semibold">Cargando...</p>
          <p className="text-text-secondary mt-2">Buscando los mejores vuelos de ida y vuelta...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-gray">
        <div className="card-modern p-12 max-w-md text-center">
          ...
          <h2 className="text-2xl font-bold text-navy mb-2">No se encontraron vuelos coincidentes</h2>
          <p className="text-text-secondary mb-6">
            {outboundFlights.length === 0 ? 'No hay vuelos de ida' : 'No hay vuelos de vuelta'}
          </p>
          <button
            onClick={() => router.push('/es/')}
            className="px-6 py-3 bg-primary text-navy font-semibold rounded-lg hover:bg-primary-dark transition-colors"
  ...
  // Calculate price based on selected cabin
  ...
  return (
    <div className="min-h-screen bg-background-gray py-8">
      <div className="container-custom">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-navy mb-4">Seleccionar vuelos de ida y vuelta</h1>
          <div className="flex flex-wrap gap-4 text-sm text-text-secondary">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>{passengers} pasajero(s)</span>
            </div>
          </div>
        </div>
          ...
          {selectedOutbound && selectedInbound && (
            <div className="card-modern p-6 sticky bottom-6 bg-white shadow-2xl">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                  <p className="text-sm text-text-secondary mb-1">Precio total ({passengers} pasajero(s))</p>
