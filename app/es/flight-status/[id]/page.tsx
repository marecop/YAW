...
export default function FlightStatusDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'IN_AIR': return 'bg-green-500'
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      ...
      case 'GATE_CLOSED': return 'Puerta Cerrada'
      case 'IN_AIR': return 'En Vuelo'
      case 'ARRIVED': return 'Aterrizado'
  ...
  if (!flight) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-xl text-gray-500">No se encontró información del vuelo</p>
      </div>
    )
  }
            ...
            <div className="absolute bottom-8 left-4 right-4 md:left-auto md:right-8 md:w-[500px] z-[1000] shadow-2xl rounded-xl overflow-hidden bg-white/95 backdrop-blur-sm border border-gray-100">
                ...
                <div className="p-6">
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm border-t border-gray-100 pt-4">
                        <div>
                            <div className="text-gray-500 mb-1">Salida Programada</div>
                            <div className="font-semibold text-lg">{format(new Date(flight.scheduledDeparture), 'HH:mm')}</div>
                        </div>
                        <div className="text-right">
                            <div className="text-gray-500 mb-1">Llegada Programada</div>
                        ...
                        <div>
                            ...
                            {(flight.status === 'IN_AIR' || flight.status === 'ARRIVED') ? (
                                <>
                                    <div className="text-gray-500 mb-1">Salida Real</div>
                                ...
                                <>
                                    <div className="text-gray-500 mb-1">Salida Estimada</div>
                        ...
                        <div className="text-right">
                            ...
                            {flight.status === 'ARRIVED' ? (
                                <>
                                    <div className="text-gray-500 mb-1">Llegada Real</div>
                                ...
                                <>
                                    <div className="text-gray-500 mb-1">Llegada Estimada</div>
                    ...
                    <div className="mt-6 pt-4 border-t border-gray-100 flex justify-between items-center text-xs text-gray-500">
                        <div>
                            <span className="font-medium text-gray-700">Avión:</span> {flight.aircraftType}
                        </div>
                        <div>
                            <span className="font-medium text-gray-700">Matrícula:</span> {flight.aircraftRegistration}
                        </div>
                        <div>
                            <span className="font-medium text-gray-700">Clima:</span> {flight.weatherOrigin} → {flight.weatherDestination}
                        </div>
                    </div>
                </div>
            </div>
