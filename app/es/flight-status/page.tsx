...
function FlightStatusContent() {
  ...
  useEffect(() => {
    // If date param is present (e.g. from lookup page), fetch flights for that date
    setLoading(true)
    setFlights([])
    setHasMore(false)
    setNextOffset(0)
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case 'SCHEDULED': return { label: 'Programado', color: 'bg-gray-100 text-gray-800', icon: Clock }
      case 'PREPARING': return { label: 'Preparando', color: 'bg-blue-50 text-blue-600', icon: Clock }
      case 'DELAYED': return { label: 'Retrasado', color: 'bg-yellow-100 text-yellow-800', icon: AlertTriangle }
      case 'CANCELLED': return { label: 'Cancelado', color: 'bg-red-100 text-red-800', icon: XCircle }
      case 'BOARDING': return { label: 'Embarcando', color: 'bg-green-100 text-green-800', icon: Plane }
      case 'GATE_CLOSED': return { label: 'Puerta Cerrada', color: 'bg-orange-100 text-orange-800', icon: Clock }
      case 'IN_AIR': return { label: 'En Vuelo', color: 'bg-blue-100 text-blue-800', icon: Plane }
      case 'ARRIVED': return { label: 'Aterrizado', color: 'bg-green-100 text-green-800', icon: CheckCircle }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Estado de Vuelos</h1>
          <p className="mt-2 text-gray-600">Información en tiempo real del estado de los vuelos</p>
        ...
        <div className="bg-white p-4 rounded-lg shadow mb-6 flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
                <label htmlFor="search" className="block text-sm font-medium text-gray-700 mb-1">Buscar vuelo</label>
                <input
                    type="text"
                    id="search"
                    placeholder="Número de vuelo / Ciudad / Código de aeropuerto"
            ...
            <div className="sm:w-64">
                ...
                >
                    ...
                    <option value="BOARDING">Embarcando (Boarding)</option>
                    <option value="GATE_CLOSED">Puerta Cerrada (Gate Closed)</option>
                    <option value="IN_AIR">En Vuelo (In Air)</option>
                    <option value="ARRIVED">Aterrizado (Arrived)</option>
                  ...
                  {flights.length > 0 ? (
                      ...
                      return (
                        <tr key={flight.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <Link href={`/es/flight-status/${flight.id}`} className="text-ya-yellow-600 hover:text-ya-yellow-900">
                              Detalles
                            </Link>
                          </td>
                        </tr>
                    ...
                    <tr>
                        <td colSpan={7} className="px-6 py-10 text-center text-gray-500">
                            No se encontraron vuelos que coincidan con los criterios
                        </td>
                    </tr>
                  )}
                </tbody>
