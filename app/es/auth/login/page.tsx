...
function LoginForm() {
  ...
  // Si ha iniciado sesión, redirigir (pero esperar a que AuthContext se inicialice)
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        // Redirigir a la URL de redirección o página de inicio
        router.push(redirectUrl)
      } else {
        setError('Inicio de sesión fallido, por favor verifique su correo electrónico y contraseña')
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          ¿No tiene una cuenta?{' '}
          <Link href="/es/auth/register" className="font-medium text-ya-yellow-600 hover:text-ya-yellow-500">
            Regístrese para una nueva cuenta
          </Link>
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm">
                <a href="/es/auth/forgot-password" className="font-medium text-ya-yellow-600 hover:text-ya-yellow-500">
          ...
          <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm font-medium text-blue-900 mb-2">Cuenta de demostración</p>
            <p className="text-xs text-blue-700">Correo: john@example.com</p>
