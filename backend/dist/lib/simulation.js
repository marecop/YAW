...
exports.ensureDailyFlights = ensureDailyFlights;
exports.updateFlightStatuses = updateFlightStatuses;
const prisma_1 = require("./prisma");
// 飛機註冊號前綴映射
const AIRLINE_REG_PREFIX = {
    'Yellow Airlines': 'B',
    'Cathay Pacific': 'B',
    'China Southern': 'B',
    'Emirates': 'A6',
...
// 獲取航空公司對應的註冊號前綴
function getRegPrefix(airline) {
    for (const [key, prefix] of Object.entries(AIRLINE_REG_PREFIX)) {
        if (airline.includes(key))
            return prefix;
    }
    return 'B'; // Default for HK based or generic
}
// 生成隨機註冊號
function generateRegistration(airline, usedRegs) {
...
// 確保當天的航班實例已創建
async function ensureDailyFlights(date = new Date()) {
    ...
    // 檢查已存在的實例
    const existingInstances = await prisma_1.prisma.flightInstance.findMany({
    ...
    // 模擬飛機池
    const aircraftPool = new Map();
    const usedRegs = new Set();
    // 將已存在的註冊號加入已使用列表，避免重複
    existingInstances.forEach(i => {
        if (i.aircraftRegistration)
            usedRegs.add(i.aircraftRegistration);
    });
    // 識別往返航班對 (僅在新實例中)
    const flightPairMap = new Map();
    for (let i = 0; i < instances.length; i++) {
        const outbound = instances[i];
        if (flightPairMap.has(i.toString()) || Array.from(flightPairMap.values()).includes(i.toString()))
            continue;
        // 找回程
        for (let j = i + 1; j < instances.length; j++) {
            const inbound = instances[j];
            if (Array.from(flightPairMap.values()).includes(j.toString()))
                continue;
    ...
    for (let i = 0; i < instances.length; i++) {
        ...
        if (instance.aircraftRegistration)
            continue;
        let assignedReg = null;
        // 嘗試從池中找可用飛機
        const suitableAircraftReq = Array.from(aircraftPool.entries()).find(([reg, state]) => {
            if (!state.airline.includes(instance.airline) && !instance.airline.includes(state.airline))
                return false;
            if (state.location !== instance.origin)
                return false;
            const minGap = 1.5 * 60 * 60 * 1000;
            if (instance.scheduledDeparture.getTime() - state.availableTime < minGap)
                return false;
            return true;
        });
    ...
    for (let i = 0; i < dbData.length; i += BATCH_SIZE) {
        ...
        await prisma_1.prisma.flightInstance.createMany({
            data: batch
        });
    }
...
// 更新當天航班狀態
async function updateFlightStatuses(date = new Date()) {
    const startOfDay = new Date(date);
    startOfDay.setHours(0, 0, 0, 0);
    const now = new Date();
    ...
    for (const instance of instances) {
        ...
        if (newStatus === 'SCHEDULED' &&
            ...
            if (Math.random() < 0.2) {
                ...
                newActualDep = null;
                newActualArr = null;
                await prisma_1.prisma.flightInstance.update({
