...
router.post('/', async (req, res) => {
    try {
        const { flightId, flightDate, cabinClass, seatNumber, passengerName, passengerEmail, passengerPhone, passportNumber, nationality, dateOfBirth, memberNumber, totalPrice, userEmail } = req.body;
        console.log('📥 收到預訂請求:', {
            flightId,
            flightDate,
            cabinClass,
        ...
        if (!flight) {
            ...
            const sampleFlights = await prisma_1.prisma.flight.findMany({
                take: 5,
...
router.get('/', async (req, res) => {
    try {
        ...
        let bookings;
        if (bookingNumber) {
            bookings = await prisma_1.prisma.booking.findMany({
