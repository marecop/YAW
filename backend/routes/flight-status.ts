...
router.get('/', async (req, res) => {
  try {
    ...
    const limit = limitParam ? Math.max(1, Math.min(parseInt(limitParam as string, 10) || 25, 200)) : null
    const offset = offsetParam ? Math.max(0, parseInt(offsetParam as string, 10) || 0) : 0
    const search = (searchParam || '').toString().trim()
    const status = statusParam && statusParam !== 'ALL' ? statusParam as string : null
