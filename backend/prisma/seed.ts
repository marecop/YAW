...
// 航空公司代码映射
const airlineMapping: Record<string, {name: string, code: string, logo: string, hubs: string[]}> = {
...
// 根據航程選擇合適的機型
function getSuitableAircraft(distanceLevel: 'short' | 'medium' | 'long'): string {
...
// 獲取隨機的早晨時間
function getRandomMorningTime(): string {
...
// 獲取隨機的下午時間
function getRandomAfternoonTime(): string {
...
// 獲取隨機的晚上時間
function getRandomEveningTime(): string {
...
async function main() {
  ...
  const getUniqueFlightNumber = (prefix: string) => {
    let fn = ''
    do {
  ...
  for (const hub of yaHubs) {
    for (const route of yaRoutes) {
      if (hub === route.to) continue // 避免原地飛
      ...
      const createYA = (dep: string, arr: string, flightNumOffset: number) => {
  ...
  if (fs.existsSync(airlinesDir)) {
    ...
    for (const file of files) {
      if (file.includes('yellow')) continue // 跳过 Yellow Airlines JSON，因为我们上面已经生成了
  ...
  for (let i = 0; i < allFlights.length; i += batchSize) {
    ...
    await Promise.all(batch.map(flight => 
      prisma.flight.create({ data: flight })
    ))
